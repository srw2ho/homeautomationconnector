...
# B1 -> IO27
# B2 -> IO17
# B3 -> IO24: Daikin WP: Climate On
# B4 -> IO16: PV Surplus (300 Watt as Dry Contact)

# OutPut-LED -> IO23

# 4 fach Relais:
# IN1-> IO07 (Daikin WP SmartGrid_0)
# IN2-> IO08 (Daikin WP SmartGrid_1)
# IN3-> IO11 (Growatt WR:FAN WR für Kühlung)
# IN4-> IO09 (Solar-Kontakt Daikin WP)
...
class GPIODeviceHomeAutomation(object):
    ...
    def initialize_gpio(self):
        try:
            ...
            # self._button_23 = Button(pull_up=None, active_state=False, pin=23)
            # B3 -> IO24 : second Relais from Left (Daikin WP: Climate On )
    ...
    def set_SolarContact(self, state: bool) -> bool:
        ...
        if state:
            if not self._digitalout_09.is_active:
                self._digitalout_09.on()
        else:
            if self._digitalout_09.is_active:
                self._digitalout_09.off()
    ...
    def button_16_Activate(self):
        ...
        logger.info(f"_button_16-Activate: Value:{self._button_16.value}")

    def button_16_Deactivate(self):
        # B4 -> IO16 : (Inverter PACToGrid < 300 Watt as Cry Contact)
        logger.info(f"_button_16-DeActivate: Value:{self._button_16.value}")

    def button_17_Activate(self):
        logger.info(f"_button_17-Activate: Value:{self._button_17.value}")

    def button_17_Deactivate(self):
        logger.info(f"_button_17-DeActivate: Value:{self._button_17.value}")

    def button_27_Activate(self):
        logger.info(f"_button_27-Activate: Value:{self._button_27.value}")

    def button_27_Deactivate(self):
        logger.info(f"_button_27-DeActivate: Value:{self._button_27.value}")

    def button_24_Activate(self):
        logger.info(f"_button_24-Activate: Value:{self._button_24.value}")

    def button_24_Deactivate(self):
        logger.info(f"_button_24-DeActivate: Value:{self._button_24.value}")
